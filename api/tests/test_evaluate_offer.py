...
def test_evaluate_offer_happy_path_counter(client):
    ...
    # (2300 + 2400) / 2 = 2350
    r = client.post("/api/evaluate-offer", json=_body(offer=2300.00), headers=AUTH)
...
def test_evaluate_offer_instant_accept_at_target(client):
    ...
    r = client.post("/api/evaluate-offer", json=_body(offer=2360.00), headers=AUTH)
    assert r.status_code == 200
    body = r.json()
...
def test_evaluate_offer_round3_below_floor_rejects(client):
    r = client.post("/api/evaluate-offer", json=_body(offer=2000.00, round_number=3), headers=AUTH)
    assert r.status_code == 200
    body = r.json()
